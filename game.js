...
class FruitMergeGame {
  constructor() {
    ...
    this.topComboValue = document.getElementById("topComboValue");
   ...
   */
  updateTopComboText() {
    this.topComboValue.textContent = String(this.maxCombo);
  }
  ...
  playComboReward(combo, level) {
    ...
    this.playComboRewardSound(audioContext, step, level);
  }
...
window.addEventListener("load", () => {
  ...
  // ── iOS 音频修复：首次触摸时播静音 <audio> 切到媒体音量 ──
  ...
  function resumeAudio() {
    ...
    // 播一段静音 WAV 让 iOS 音频会话切到媒体音量通道
    // 之后所有 OscillatorNode 都走媒体音量
    try {
      const sampleRate = 44100;
      const length = sampleRate * 0.08; // 0.08s silence
      const wav = new ArrayBuffer(44 + length * 2);
      const v = new DataView(wav);
      const w = (off, str) => { for (let i = 0; i < str.length; i++) v.setUint8(off + i, str.charCodeAt(i)); };
      w(0, 'RIFF'); v.setUint32(4, 36 + length * 2, true); w(8, 'WAVE');
      w(12, 'fmt '); v.setUint32(16, 16, true); v.setUint16(20, 1, true);
      v.setUint16(22, 1, true); v.setUint32(24, sampleRate, true);
      v.setUint32(28, sampleRate * 2, true); v.setUint16(32, 2, true);
      v.setUint16(34, 16, true); w(36, 'data'); v.setUint32(40, length * 2, true);
      const silent = new Audio(URL.createObjectURL(new Blob([wav], { type: 'audio/wav' })));
      silent.volume = 0.001;
      silent.play().catch(() => {});
    } catch (e) { /* ignore */ }
    ...
    if (g.audioContext) {
      ...
      if (AC) {
        g.audioContext = new AC();
        g.masterGain = g.audioContext.createGain();
        g.masterGain.connect(g.audioContext.destination);
        g.musicGain = g.audioContext.createGain();
